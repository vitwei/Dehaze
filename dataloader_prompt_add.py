...
class lowlight_loader(data.Dataset):
	...
	def __getitem__(self, index):
		...
		if ("normal" in data_lowlight_path)or("output"in data_lowlight_path)or("/data/HQ/"in data_lowlight_path)or("DIV2K"in data_lowlight_path)or("high"in data_lowlight_path)or("positive"in data_lowlight_path):
			label=torch.from_numpy(np.array(1))
		else:
			label=torch.from_numpy(np.array(0))
	
		return image_features,label,image2
